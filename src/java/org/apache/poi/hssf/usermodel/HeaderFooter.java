...
public abstract class HeaderFooter implements org.apache.poi.ss.usermodel.HeaderFooter {
    ...
    /**
     * Creates the complete footer string based on
     * the left, center, and middle strings.
     */
    private void updateHeaderFooterText(String[] parts) {
        ...
        if (center.length() < 1 && left.length() < 1 && right.length() < 1) {
            setHeaderFooterText("");
            return;
        }
    ...
    private enum MarkupTag {
        ...
        private final boolean occursInPairs;

        private MarkupTag(String sequence, boolean occursInPairs) {
            this.representation = sequence;
            this.occursInPairs = occursInPairs;
        }
