...
/**
 * One full evaluation pass over a {@link HSSFWorkbook} whose formulas keep reading the same large
 ...
 * {@link HSSFFormulaEvaluationBenchmark} measures a workbook in which each formula mostly reads its
 ...
 * plain cells.
 * <p>
 * Setup, evaluator lifetime and threading are as in {@link HSSFFormulaEvaluationBenchmark}: the
...
@Threads(1)
@State(Scope.Thread)
public class HSSFLookupEvaluationBenchmark {
    ...
    @Setup(Level.Trial)
    public void setUp() {
        wb = new LookupWorkbookBuilder(rows, tableRows, 42L).build(new HSSFWorkbook());
    ...
    public void evaluateAll() {
        HSSFFormulaEvaluator.create(wb, classifier, null).evaluateAll();
    }
}
