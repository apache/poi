	...
	public static void fuzzerTestOneInput(byte[] input) throws Exception {
		...
		for (FileHandler handler : handlers) {
			...
			try {
				...
				if (!ExceptionUtils.getStackTrace(e).contains("Can't connect to X11 window server")) {
					throw e;
				}
			} catch (NoClassDefFoundError e) {
				// POI cannot fix it if there is no DISPLAY
				if (!ExceptionUtils.getStackTrace(e).contains("Could not initialize class java.awt.GraphicsEnvironment$LocalGE")) {
					throw e;
				}
			} catch (InternalError e) {
				// POI cannot fix it if the font-system is not fully installed, so let's ignore
				// this for fuzzing
				if (!ExceptionUtils.getStackTrace(e).contains("Fontconfig head is null")) {
					throw e;
				}
